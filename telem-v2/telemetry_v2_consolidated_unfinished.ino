...
struct TelemetryRecord_t
{
  ...
  // GPS fields are fixed-point so the CSV writer stays simple and compact.
  // gps_valid = 1 means the GPS location/date/time fields are valid.
  uint8_t gps_valid;
  int32_t gps_lat_e7;       // GPS latitude  * 10,000,000
  int32_t gps_long_e7;       // GPS longitude * 10,000,000
  int32_t gps_alt_cm;       // GPS altitude in centimeters
  ...
  uint8_t gps_minute;
  uint8_t gps_second;
  uint8_t gps_centisecond;  // 0-99, from TinyGPS++ gps.time.centisecond()
...
{
  rec.gps_valid = gps.location.isValid() ? 1 : 0;

  rec.gps_lat_e7 = rec.gps_valid ? (int32_t)(gps.location.lat() * 10000000.0) : 0;
  rec.gps_long_e7 = rec.gps_valid ? (int32_t)(gps.location.lng() * 10000000.0) : 0;

  rec.gps_alt_cm = gps.altitude.isValid() ? (int32_t)(gps.altitude.meters() * 100.0) : 0;
  rec.gps_speed_kmph_x100 = gps.speed.isValid() ? (uint16_t)(gps.speed.kmph() * 100.0) : 0;
  rec.gps_hdop_x100 = gps.hdop.isValid() ? (uint16_t)gps.hdop.value() : 0;
  rec.gps_sats = gps.satellites.isValid() ? (uint8_t)gps.satellites.value() : 0;
...
void dumpLogCsv()
{
  ...
  Serial.println("timestamp_ms,current_mA,voltage_mV,ax_x100,ay_x100,az_x100,amag_x100,gps_valid,gps_lat,gps_long,gps_alt_m,gps_speed_kmph,gps_hdop,gps_sats,gps_time_utc,gps_centisecond");
  ...
  while (f.available() >= (int)sizeof(TelemetryRecord_t)) {
    ...
    Serial.printf("%lu,%d,%ld,%d,%d,%d,%u,%u,%.7f,%.7f,%.2f,%.2f,%.2f,%u,%02u:%02u:%02u,%02u\n",
      ...
      rec.gps_alt_cm / 100.0,
      rec.gps_speed_kmph_x100 / 100.0,
      rec.gps_hdop_x100 / 100.0,
      rec.gps_sats,
      rec.gps_hour,
      rec.gps_minute,
      rec.gps_second,
...
bool writeSDCsvHeader(const char *path)
{
  ...
  f.println("timestamp_ms,current_mA,voltage_mV,ax_x100,ay_x100,az_x100,amag_x100,gps_valid,gps_lat,gps_long,gps_alt_m,gps_speed_kmph,gps_hdop,gps_sats,gps_time_utc,gps_centisecond");
  f.close();
  return true;
}

bool makeTimestampLogFilename(char *out, size_t outSize)
{
  struct tm timeinfo;

  // This only works if system time has been set by GPS, NTP, RTC, or serial command.
  if (!getLocalTime(&timeinfo)) {
    return false;
  }

  // Filename format: /year_month_day_hour_minute.csv
  // Uses 24-hour time to avoid AM/PM and invalid filename characters like ':'.
  snprintf(
    out,
    outSize,
    "/%04d_%02d_%02d_%02d_%02d.csv",
...
bool selectNextSDLogFile()
{
  ...
  // First try timestamp-based filename.
  if (makeTimestampLogFilename(g_sdLogFile, sizeof(g_sdLogFile))) {
    // If exact minute already exists, add _001, _002, etc.
    if (!SD.exists(g_sdLogFile)) {
      if (!writeSDCsvHeader(g_sdLogFile)) return false;
    ...
    // Remove ".csv"
    char *dot = strstr(baseName, ".csv");
    if (dot) *dot = '\0';

    for (uint16_t i = 1; i <= SD_MAX_LOG_FILES; i++) {
      snprintf(g_sdLogFile, sizeof(g_sdLogFile), "%s_%03u.csv", baseName, i);
    ...
    Serial.println("No free timestamped SD log filename left.");
    return false;
  }

  // Fallback if time has not been set.
  for (uint16_t i = 1; i <= SD_MAX_LOG_FILES; i++) {
    ...
    if (!SD.exists(g_sdLogFile)) {
      ...
      Serial.printf("Time unavailable. New fallback SD log file: %s\n", g_sdLogFile);
      return true;
    }
  }

  Serial.println("No free SD log filename left.");
  return false;
}
...
bool appendRecordSD(const TelemetryRecord_t &rec)
{
  ...
  f.printf("%lu,%d,%ld,%d,%d,%d,%u,%u,%.7f,%.7f,%.2f,%.2f,%.2f,%u,%02u:%02u:%02u,%02u\n",
    (unsigned long)rec.timestamp_ms,
    rec.current_mA,
    (long)rec.voltage_mV,
    rec.accel_x_mps2_x100,
...
void setup()
{
  ...
  // GPS
  gpsSerial.begin(GPS_BAUD, SERIAL_8N1, RXD2, TXD2); // Initialize
...
void loop()
{
  ...
  // GPS: consume incoming NMEA bytes so TinyGPS++ keeps the latest parsed values.
  // The parsed GPS values are written into each TelemetryRecord_t below.
  serviceGps();

  // GPS Diagnostic check: If 5 seconds pass and the library hasn't processed any characters,
  ...
  if (appendRecordSD(rec)) {
    Serial.printf(
      "SD LOG %s t=%lu ms I=%d mA V=%ld mV Ax=%d Ay=%d Az=%d Mag=%u GPS=%u sats=%u raw=%d\n",
      g_sdLogFile,
      (unsigned long)rec.timestamp_ms,
      rec.current_mA,
      (long)rec.voltage_mV,
